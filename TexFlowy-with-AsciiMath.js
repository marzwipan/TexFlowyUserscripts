...
// @grant        GM.addStyle
// @require      https://cdn.jsdelivr.net/npm/katex@0.15.2/dist/katex.min.css
...
//		load_css();
	 ...
	 */
	function handle_node(node,macros) {
 ...
 //              node.style.display = 'none';
	 ...
	 */
	function load_css() {
		 const katex_css = GM.getResourceText("KATEX_CSS");
         const katex_css_trafo = transformCss(katex_css);

        GM.addStyle(katex_css_trafo);
	}
 ...
 */
	 ...
	 */
	function hide_raw() {
		GM.addStyle('.name .innerContentContainer.has-latex  { display:none } ');
		GM.addStyle('.name .content.has-latex {    display:flex; flex: 0 0 auto; padding-right: 0px; min-height:0px; height:0px} ');

		GM.addStyle('.name--focused .has-latex .innerContentContainer { display:inline} ');
        GM.addStyle('.name.name--focused .has-latex { display: flex; flex: 1 1 auto; padding-right: 10px; height:auto} ');
        GM.addStyle('.name--focused .rendered-latex { display:none } ')

		// add a background to make the raw part look clearer
//		GM_addStyle('.name--focused .has-latex { background: #eee } ');

        GM.addStyle('.notes .innerContentContainer.has-latex {display: none}');
        GM.addStyle('.notes .active .innerContentContainer.has-latex  { display:inline} ');

        GM.addStyle('.notes .rendered-latex { overflow: visible ;\n display: block; \n  max-height: none ; \n height: auto ;  \n}');
        GM.addStyle('.notes .active ~ .rendered-latex { display:none }');
//        GM_addStyle('.notes .content { color: rgb(0,0,0) !important }');
//		GM.addStyle('.newMobileDesign .name .innerContentContainer.has-latex  { display:none } ');
//		GM.addStyle('.newMobileDesign .name .content.has-latex {    display:flex; flex: 0 0 auto; padding-right: 0px; min-height:0px; height:0px} ');
//
//		GM.addStyle('.newMobileDesign .name--focused .has-latex .innerContentContainer { display:inline} ');
  //      GM.addStyle('.newMobileDesign .name.name--focused .has-latex { display: flex; flex: 1 1 auto; padding-right: 10px; height:auto} ');
   //     GM.addStyle('.newMobileDesign .name--focused .rendered-latex { display:none } ')
	//	GM.addStyle('.newMobileDesign .page { background-color: rgb(241, 99, 28)}');
